function model = xls2modelv2(fileName,biomassRxnEquation)
% xls2modelv2 Writes a model from Excel spreadsheet.
%
% model = xls2modelv2(fileName,metFileName)
%
% INPUT
...
p = what; curPath = p.path;
if nargin < 1
    [fileName, newPath] = uigetfile({'*.xls';'*.xlsx'});
    cd(newPath);
end
...
if isunix
    ...
    % the following line was commented out by Hadi Nazem-Bokaee on 10-07-2013
...
% commented out by Hadi Nazem-Bokaee on 3-13-2012 - start
% [r,c] = size(Numbers);
% if c >= 1
...
% addition of a negative sign by Hadi Nazem-Bokaee on 3-13-2012    
...
% end
% commented out by Hadi Nazem-Bokaee on 3-13-2012 - end
model = createModelv2(rxnAbrList,rxnNameList,rxnList,revFlagList,lowerBoundList,upperBoundList,subSystemList,grRuleList);
if size(Numbers,2)>=5
    ConfidenceScore = Numbers(:,5);
    model.confidenceScores = regexprep(cellstr(num2str(ConfidenceScore)),'NaN| ','');
% commented out by Hadi Nazem-Bokaee on 3/13/2012
% else
